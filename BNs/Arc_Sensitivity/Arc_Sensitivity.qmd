...
I am not sure it will work for that, we might have to create a slightly more complex toy network that can test this. 
The following code works when there is a binary child node (with negative response being the last value). 
The value assigned to the P(child=No|parent=False) is 1 to ensure that the CPT sums to 1.
...
for(row in  1:nrow(arcsOfInterest)) {
  ...
  # Redefine the CPT for the node of interest to add the switch node levels
  interestCPT <- c(origNodeCPT, 0, 1)
  dim(interestCPT) <- c(length(origNodeCPT),length(newNodeCPT))
  dimnames(interestCPT)  <- list(names(origNodeCPT), names(newNodeCPT))
...
graphviz.chart(updated_fitted_net,
                ...
                strip.bg = "lightskyblue")
```
Current issue is that it appears that the marginal probabilities of TPPR6 have swapped from 1,0 to 0,1.  
Must be during my setting of the CPT so need to think about it more.
