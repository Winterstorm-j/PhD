...
sapply(
  c("dplyr","magrittr","bnlearn","gRain","BiocManager"),
  FUN=function (x){
...
```{r}
# filepath for render 
# setwd("./PhD/BNs/Arc_Sensitivity") #  file path when running manually
...
```{r}
arcs <- as.data.frame(arcs(originalNetwork))
nrow <- length(arcs[, 1])

arcs %>% kable(row.names = TRUE)
```

Taking into account Duncan's thoughts, we need a way to decide which nodes require the addition of a gatekeeping node to switch it on or off. We could find the leaf nodes but they wont neccessarily be results nodes especially if we have a BN with multiple tests of the item. If the leaf nodes are results then they would have transfer nodes as parents so would be the arcs we want to adjust. But it would not include transfer nodes as parents of other transfer nodes or test nodes.

Instead, we can choose based on the arc table any arcs which dont have HpHd, as a first filter. Addtionally, Aany direct descendant of HpHd will also be a proposed activity. Any child node of these activity nodes would have a required arc so the arc should be excluded.

## Filtering the arc table to find nodes of interest

```{r}
# exclude arcs that originate from "HpHd" or that originate from any child of "HpHd"
activityNodes <- arcs %>% 
    filter(from == "HpHd") %>% 
    pull(to)

arcsOfInterest <- arcs %>%
  filter(!(from == "HpHd" | from %in% activityNodes))

arcsOfInterest %>% kable(row.names = TRUE)
```

The parent nodes of these remaining arcs would be candidates for having a gatekeeping boolean node added as an additional parent.
Defining a new conditional table for the node of interest that becomes the new child node may be interesting. 
Where this node has no other parents it is relatively straight forward with only a consideration for how many states exist. 
Otherwise, this needs some thinking.  It may need an if statement with a check for existing parents to be included.

I have tried to keep the incoroporation of the new parent into the node of interest as general as possible to account for existing parents.  
While TPPR6 has no existing parents, TPPR5 has one (Activity3). This is a good test of the generalisation.
The following code works when there is a binary child node (with negative response being the last value). 
The value assigned to the P(child=No|parent=False) is 1 to ensure that the CPT sums to 1. Fix needed for multiple parent nodes or >2 states.

```{r}
nodeCount <- nrow(arcsOfInterest)
new_net <- bn.net(originalNetwork)

# Create a list of all CPTs
all_cpts <- lapply(originalNetwork, "[[", "prob")

for(row in  1:nrow(arcsOfInterest)) {
  print(row)
  nodeOfInterest <- arcsOfInterest[[row,"from"]]

  # retrieve exisitng CPT of node of interest
  origNodeCPT <- originalNetwork[nodeOfInterest][[1]]$prob
  print(glue("Original CPT of {nodeOfInterest}"))
  print(origNodeCPT)

  # Clarify which node we are working on 
  print(glue("Create new parent of {nodeOfInterest}"))

  newName <- glue("{nodeOfInterest}a")
  # Add a new node with newly created name above and set a new arc with original node as child
  new_net <- add.node(new_net, newName)
  new_net <- set.arc(new_net, from = newName, to = nodeOfInterest)

  # Define the CPT for switch node with states "true" and "false"
  newNodeCPT <- cpt(newName, levels = c("true", "false"), values=c(1,0))

  # Redefine the CPT for the node of interest to add the switch node levels for each level of child node states
  interestCPT <- c(origNodeCPT, rep(0, length(origNodeCPT)-1), 1)
  
  # if the original node had no parents - it needs to be named witht he node name, if at least one parent - it is already named
  if(dim(as.matrix(origNodeCPT))[2]>1){
      dim(interestCPT) <- c(dim(as.matrix(origNodeCPT)),length(newNodeCPT))
      dimnames(interestCPT)  <- c(dimnames(origNodeCPT), dimnames(newNodeCPT))
    } else{
      dim(interestCPT) <- c(dim(as.matrix(origNodeCPT))[1],length(newNodeCPT))
      dimnames(interestCPT)  <- c(dimnames(origNodeCPT), dimnames(newNodeCPT))
      names(dimnames(interestCPT)) <- c(nodeOfInterest, newName)
    }
  
  
  all_cpts[[ newName ]] <- newNodeCPT
  all_cpts[[ nodeOfInterest ]] <- interestCPT
}
```

Fitting is commented until CPT of parented node is worked out (to allow rendering)
```{r}

# Fit the network with the new CPTs
#updated_fitted_net <- custom.fit(new_net, dist = all_cpts)
```

```{r}
# par(bg="white")
# graphviz.chart(updated_fitted_net,
#                 type = "barprob",
#                 grid = TRUE, 
#                 bar.col = "darkgreen",
#                 strip.bg = "lightskyblue")
...
# *Old code below set as block code to keep but not run*
...
> ```{r}
> #bnlearn::BF(numerator, denom) computes bayes factor between two networks # P(Child∣Remaining Parents)= ∑ P(Child∣Parents,Remaining Parents)⋅P(Parents)
> >
...
![](HuginMoveArcs.png){#fig-hugin-arcsmoved}
